...
def gerar_srt_simples(texto: str, audio_path: Path) -> Path:
    ...
    dur_por_grupo = max(duracao / len(grupos), 0.8)
    ...
    with open(srt_path, "w", encoding="utf-8") as f:
        for i, grupo in enumerate(grupos, 1):
            ...
            f.write(
                f"{i}\n"
                f"{formatar_tempo_srt(inicio)} --> {formatar_tempo_srt(fim)}\n"
                f"{grupo}\n\n"
            )
            inicio = fim
...
def montar_slideshow(frames: List[Path], audio_path: Path, srt_path: Path, duracao_audio: float) -> Path:
    ...
    # zoom leve estilo news/short documentary
